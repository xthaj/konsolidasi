<x-one-panel-layout>
    ...
    <div class="my-4 flex">
        <h1 class="text-lg font-semibold mr-2">Upload/Update Data Harmonisasi</h1>
        ...
        <div class="mt-4" x-data="{ loading: false, showError: false, sizeError: false, typeError: false, levelError: false, maxSizeMB: 5 }">
            ...
            "
                    ...
                    <div x-show="loading" class="flex items-center">
                        ...
                        <span class="text-sm text-gray-600 ml-2">Loading... Harap untuk tidak menutup tab</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- Harga Upload Section -->
    <div class="my-4 flex">
        <h1 class="text-lg font-semibold mr-2">Upload/Update Data Harga</h1>

        <span data-tooltip-target="tooltip-harga" data-tooltip-placement="bottom" class="material-symbols-rounded">
            info
        </span>

        <div id="tooltip-harga" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip">
            <ul>
                <li>1. Gunakan kode komoditas tanpa padding (contoh: 0, bukan 000)</li>
                <li>2. Untuk kode wilayah nasional, gunakan 0 (jangan dikosongkan). Proses berhenti saat kode wilayah kosong.</li>
                <li>3. Gunakan titik sebagai separator desimal (contoh: 1.23, bukan 1,23)</li>
                <li>4. File harus memiliki kolom: kd_wilayah, kd_komoditas, harga, rentang_bawah, rentang_atas</li>
                <li>5. Data harga hanya bisa diisi jika data harmonisasi sudah ada.</li>
            </ul>
            <div class="tooltip-arrow" data-popper-arrow></div>
        </div>
    </div>

    <form action="{{ route('data.upload-harga') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
            ...
            <div>
                ...
                <select id="level-harga" name="level" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-full p-2.5">
                    <option value="" selected>Pilih Level Harga</option>
                    <option value="01">Harga Konsumen Kota</option>
                    <option value="02">Harga Konsumen Desa</option>
                    <option value="03">Harga Perdagangan Besar</option>
        ...
        <div class="mt-4">
            <label class="block mb-1 text-sm font-medium text-gray-900" for="file_input_harga">Upload File</label>
            <input name="file"
                x-ref="fileInputHarga"
                class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50"
                id="file_input_harga"
                type="file"
                accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet">
        ...
        <div class="mt-4" x-data="{ loading: false, showError: false, sizeError: false, typeError: false, levelError: false, maxSizeMB: 5 }">
            <div x-show="levelError" class="text-sm mb-4 text-red-600">Pilih level harga terlebih dahulu.</div>
            <div x-show="showError" class="text-sm mb-4 text-red-600">Pilih file terlebih dahulu.</div>
            <div x-show="sizeError" class="text-sm mb-4 text-red-600">File terlalu besar. Maksimal 5MB.</div>
            <div x-show="typeError" class="text-sm mb-4 text-red-600">File harus berupa Excel (XLSX).</div>
            <div class="flex flex-col sm:flex-row sm:justify-between items-center gap-3">
                <a
                    href="{{ asset('template/template_harga.xlsx') }}"
                    download
                    class="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-primary-700 bg-white border border-primary-700 rounded-lg transition-colors duration-200 hover:bg-gray-100 focus:outline-none focus:ring-4 focus:ring-primary-300 w-full sm:w-auto">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M12 16V4m0 12l-4-4m4 4l4-4"></path>
                    </svg>
                    Download Template
                ...
                <div class="flex flex-col items-center sm:items-end gap-2 w-full sm:w-auto">
                    <x-primary-button
                        type="submit"
                        @click="
                const level = document.getElementById('level-harga').value;
                if (!level) {
                    $event.preventDefault();
                    levelError = true;
                    showError = false;
                ...
                levelError = false;
                const file = $refs.fileInputHarga.files[0];
                const allowedTypes = ['application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.ms-excel'];
                if (!file) {
                    $event.preventDefault();
                    showError = true;
    ...
    <!-- Hapus Data Harga -->
    <div class="my-4">
        <h1 class="text-lg font-semibold">Hapus Data Harga</h1>
    </div>

    <form id="delete-harga-form" action="{{ route('data.hapus-harga') }}" method="POST" x-data="{ loading: false, levelError: false }" @submit="
        const level = document.querySelector('#delete-harga-form select[name=&quot;level&quot;]').value;
        if (!level) {
            $event.preventDefault();
            levelError = true;
            return;
    ...
    ">
        ...
        <p class="mt-4 text-xs text-gray-500">Data harga, rentang bawah, dan rentang atas akan dikosongkan. Data harmonisasi tidak terpengaruh.</p>
