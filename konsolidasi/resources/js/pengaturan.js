    ...
    Alpine.data('webData', () => ({
        ...
        async init() {
            ...
            try {
                ...
                this.wilayahData = (wilayahData.provinces || []).concat(wilayahData.kabkots || []);
        ...
        async addKomoditas() {
            try {
                ...
                this.successMessage = 'Komoditas added successfully';
        ...
        async updateKomoditas() {
            try {
                const response = await fetch(`/komoditas/${this.editKomoditas.kd_komoditas}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                    ...
                    body: JSON.stringify({ nama_komoditas: this.editKomoditas.nama_komoditas }) // Only send nama_komoditas
                });
                ...
                const index = this.komoditasData.findIndex(k => k.kd_komoditas === data.kd_komoditas);
                if (index !== -1) {
                    this.komoditasData[index] = data;
                }
                this.successMessage = 'Komoditas updated successfully';
                this.$dispatch('open-modal', 'success-update-bulan-tahun');
                this.$dispatch('close');
            } catch (error) {
                this.failMessage = 'An unexpected error occurred';
        ...
        deleteKomoditas(kd_komoditas) {
            const komoditas = this.komoditasData.find(k => k.kd_komoditas === kd_komoditas);
            if (!komoditas) return;
            ...
            this.confirmAction = async () => {
                try {
                    ...
                    if (!response.ok) {
                        this.failMessage = data.message || 'Failed to delete komoditas';
                        this.failDetails = data.details || null;
                        this.$dispatch('open-modal', 'fail-update-bulan-tahun');
                        return;
                    }

                    this.komoditasData = this.komoditasData.filter(k => k.kd_komoditas !== kd_komoditas);
                    this.successMessage = data.message || 'Komoditas deleted successfully';
                    this.$dispatch('open-modal', 'success-update-bulan-tahun');
                } catch (error) {
                    this.failMessage = 'An unexpected error occurred';
                    this.failDetails = { error: error.message };
